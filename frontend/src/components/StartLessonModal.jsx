...
// Start a class session, or (via `initial`) edit a running one's restrictions.
// A session has two restriction modes:
//   monitor — students keep normal district filtering; the session exists for
//             visibility (live view, history, ClassPulse)
//   focus   — students can only reach the listed sites (scene-able)
export default function StartLessonModal({ onStart, onClose, loading, initial = null }) {
  const editing = !!initial;
  const [name,    setName]    = useState(initial?.name || '');
  const [mode,    setMode]    = useState(initial?.restriction_mode || (editing ? 'focus' : 'monitor'));
  ...
  // classes — applying one just fills the textarea, so it stays editable.
  ...
  const handleSubmit = (e) => {
    ...
    onStart({
      ...
      allowed_domains: mode === 'focus' ? parseDomains(domains) : [],
    });
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-5 border-b border-slate-100">
          <h2 className="text-lg font-bold text-slate-900">
            {editing ? 'Class restrictions' : 'Start Class'}
          </h2>
          <p className="text-sm text-slate-500 mt-0.5">
            {editing
              ? 'Changes apply to the running class immediately.'
              : 'Start monitoring your class — you can restrict browsing now or any time during class.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          {!editing && (
            <div>
              <label className="label">Session name <span className="font-normal text-slate-400">(optional)</span></label>
              <input
                ref={inputRef}
                className="input"
                placeholder="e.g. Period 3 — Chapter 7 Research"
                value={name}
          ...
          <div className="space-y-2">
            <label className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-colors
              ${mode === 'monitor' ? 'border-primary-400 bg-primary-50/50' : 'border-slate-200 hover:border-slate-300'}`}>
              <input
                type="radio"
                name="restriction_mode"
                checked={mode === 'monitor'}
                onChange={() => setMode('monitor')}
                className="mt-0.5"
              />
              <div>
            ...
            <label className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-colors
              ${mode === 'focus' ? 'border-primary-400 bg-primary-50/50' : 'border-slate-200 hover:border-slate-300'}`}>
              <input
                type="radio"
                name="restriction_mode"
                checked={mode === 'focus'}
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Students can only use the websites you list below.</p>
              </div>
            </label>
          </div>
            ...
            <>
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  One domain per line. Leaving this empty blocks all browsing.
                </p>
              </div>
                    ...
                    <div className="space-y-2">
                      <input
                        ...
                        onKeyDown={e => {
                          ...
                          // surrounding form (which would start the class).
                          if (e.key === 'Enter') {
                            e.preventDefault();
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" className="btn-primary flex-1" disabled={loading}>
              {loading ? (editing ? 'Applying…' : 'Starting…') : (editing ? 'Apply' : '▶ Start Class')}
            </button>
          </div>
        </form>
      </div>
