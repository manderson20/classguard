...
// blocked row to show exactly why, reusing the same step-by-step trace
  ...
  return (
    <tr className="bg-slate-50">
      <td colSpan={colSpan} className="px-4 py-3 text-xs">
        {isLoading ? (
          <span className="text-slate-400">Checking decision trace for {domain}…</span>
        ) : error ? (
          <span className="text-red-500">Couldn't trace this: {error.message}</span>
        ) : (
