...
function SafetyAlertsSection({ appSettings, appLoading, saved, setSaved }) {
  const qc = useQueryClient();
  const [smtp, setSmtp] = useState({
    smtp_host: '', smtp_port: '587', smtp_secure: 'false',
    smtp_user: '', smtp_password: '', smtp_from: '', safety_alert_emails: '',
  });
  ...
  useEffect(() => {
    if (appSettings && Object.keys(appSettings).length) {
      setSmtp({
        ...
        safety_alert_emails: appSettings.safety_alert_emails || '',
      });
    }
  }, [appSettings]);
        ...
        {appLoading ? <div className="text-slate-400 text-sm">Loading…</div> : (
          <>
            ...
            <Field label="Alert Recipients" hint="Comma-separated emails — counselors, admins, whoever should know immediately">
              <input className="input" value={smtp.safety_alert_emails} onChange={e => setSmtp(s => ({ ...s, safety_alert_emails: e.target.value }))} placeholder="counselor@yourschool.org, admin@yourschool.org" />
            </Field>
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              {saved === 'safety-alerts' && <span className="text-green-600 text-sm font-medium">Saved!</span>}
              {testResult && (
...
const TABS = ['Branding', 'DNS & Retention', 'Monitoring', 'Safety Alerts', 'About'];
...
export default function SettingsPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {tab === 'Safety Alerts' && (
        <SafetyAlertsSection appSettings={appSettings} appLoading={appLoading} saved={saved} setSaved={setSaved} />
      )}
