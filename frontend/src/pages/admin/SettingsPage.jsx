...
// Mail relay connection only — who actually receives a safety alert lives
// under Safety Alerts > Alerting instead (its own permission key, since
// that's an application-level decision, not core server config). This is
// just the reusable SMTP infrastructure any future feature could send
// through.
function CommunicationsSection({ appSettings, appLoading, saved, setSaved }) {
  const qc = useQueryClient();
  ...
  useEffect(() => {
    if (appSettings && Object.keys(appSettings).length) {
      setSmtp({
        ...
        smtp_secure:   appSettings.smtp_secure    || 'false',
        smtp_user:     appSettings.smtp_user     || '',
        smtp_password: appSettings.smtp_password || '',
  ...
  return (
    <Section title="Mail Server (SMTP)">
      <p className="text-sm text-slate-600 mb-4">
        The outgoing mail relay used by any ClassGuard feature that sends email — currently Safety Alerts'
        urgent notifications. Configure who receives those under Safety Alerts &gt; Alerting.
      </p>
      {appLoading ? <div className="text-slate-400 text-sm">Loading…</div> : (
        <>
          <Field label="SMTP Host">
            <input className="input" value={smtp.smtp_host} onChange={e => setSmtp(s => ({ ...s, smtp_host: e.target.value }))} placeholder="smtp.gmail.com" />
          </Field>
          <Field label="SMTP Port">
            <input className="input" value={smtp.smtp_port} onChange={e => setSmtp(s => ({ ...s, smtp_port: e.target.value }))} placeholder="587" />
          </Field>
          <Field label="Use TLS (secure)">
            <select className="input" value={smtp.smtp_secure} onChange={e => setSmtp(s => ({ ...s, smtp_secure: e.target.value }))}>
              <option value="false">No (STARTTLS on 587)</option>
              <option value="true">Yes (implicit TLS on 465)</option>
            </select>
          </Field>
          <Field label="SMTP Username">
            <input className="input" value={smtp.smtp_user} onChange={e => setSmtp(s => ({ ...s, smtp_user: e.target.value }))} />
          </Field>
          <Field label="SMTP Password">
            <input type="password" className="input" value={smtp.smtp_password} onChange={e => setSmtp(s => ({ ...s, smtp_password: e.target.value }))} />
          </Field>
          <Field label="From Address">
            <input className="input" value={smtp.smtp_from} onChange={e => setSmtp(s => ({ ...s, smtp_from: e.target.value }))} placeholder="alerts@yourschool.org" />
          </Field>
...
export default function SettingsPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {tab === 'Communications' && (
        <CommunicationsSection appSettings={appSettings} appLoading={appLoading} saved={saved} setSaved={setSaved} />
      )}
