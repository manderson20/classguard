...
- **Zabbix agent 2 monitoring for cluster nodes** — new `infrastructure/zabbix/` with an installer (`install-zabbix-agent2.sh`), agent UserParameters, and an importable template (`templates/classguard_node_by_agent2.yaml`) that monitors each node separately: all `/metrics` application stats parsed into per-item graphs/triggers, FreeRADIUS/keepalived/update-watcher service state, 1812/udp listening, EAP cert expiry from disk, failed-update markers, VIP ownership cross-checked against the node's VRRP claim, and Docker discovery of every classguard-* container (running/OOM/restarts/memory). HA-aware: the active server is visible per node via `vrrp_state`, with failover, FAULT, and peer-offline triggers.
- **/metrics: RADIUS, certificate and replication metrics** — the Zabbix endpoint now also reports auth accepts/rejects (5m), active sessions, device counts by status, online NAS count, ACME cert days remaining, this node's DB replication lag, and the app version; the generated HTTP-agent template gains matching triggers (peer offline, cert expiring, sustained rejects).
